...
│   ├── ParallelConvolution.kt  # параллельная свёртка (4 стратегии)
│   ├── Kernels.kt              # каталог стандартных ядер
│   └── ImageIO.kt              # загрузка/сохранение через javax.imageio
├── task1/                      # CLI задачи 1 (kotlinx-cli)
├── task2/                      # CLI задачи 2 (стратегии + число потоков)
├── benchmarks/                 # JMH-бенчмарки (sequential + parallel)
...
```bash
# с дефолтами: gaussian, by-rows, потоков = availableProcessors
make task2

# выбор стратегии и числа потоков
make task2 ARGS="-i samples/img4.jpg -o out/parallel.png -k gaussian -s by-rows -t 8"
...
| ---- | ---------------- | ----------------------------------- | --------------------------------------------------------- |
| `-i` | `--input`        | `samples/img1.jpg`                  | путь к входной картинке                                   |
| `-o` | `--output`       | `out/task2.png`                     | путь к выходной картинке                                  |
| `-k` | `--kernel`       | `gaussian`                          | имя ядра                                                  |
| `-s` | `--strategy`     | `by-rows`                           | стратегия: `per-pixel` \| `by-rows` \| `by-columns` \| `grid` |
| `-t` | `--threads`      | `0` (= `availableProcessors`)       | число потоков пула                                        |
...
| `gaussian-5x5`   | 5×5    | более плавное гауссово размытие                 |
| `motion-blur`    | 9×9    | размытие по диагонали (имитация движения)       |
| `edge-detection` | 3×3    | обнаружение краёв во всех направлениях          |
| `sharpen-5x5`    | 5×5    | тонкое повышение резкости                       |
| `emboss`         | 3×3    | эффект тиснения (3D-тень, bias=128)             |
| `emboss-5x5`     | 5×5    | выраженный эффект тиснения                      |
...
Подробный разбор по каждой задаче — в отдельных документах:
